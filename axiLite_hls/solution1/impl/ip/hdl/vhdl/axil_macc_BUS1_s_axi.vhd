...
entity axil_macc_BUS1_s_axi is
generic (
...
end entity axil_macc_BUS1_s_axi;
...
architecture behave of axil_macc_BUS1_s_axi is
    ...
    constant ADDR_INSTR_DATA_0 : INTEGER := 16#30#;
    constant ADDR_INSTR_CTRL   : INTEGER := 16#34#;
    ...
    signal int_instr           : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_INSTR_DATA_0 =>
                        rdata_data <= RESIZE(int_instr(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    instr                <= STD_LOGIC_VECTOR(int_instr);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_INSTR_DATA_0) then
                    int_instr(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_instr(31 downto 0));
                end if;
            end if;
        end if;
    end process;
